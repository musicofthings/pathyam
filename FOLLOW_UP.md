...
|---|---|---|
| 1 | **Gemini API key + model id** | The vision provider is rewritten and fails loudly, but **has never made a live call** — there is no key in this environment. I deliberately did not pick a replacement model id: the old default `gemini-3.7-flash` was not a real model, and guessing a second time repeats the mistake. Set `GEMINI_API_KEY`, run `client.models.list()`, put a verified id in `PATHYAM_VISION_MODEL`. |
