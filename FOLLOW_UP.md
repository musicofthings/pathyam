...
|---|---|---|
| 1 | ~~OpenRouter API key~~ — **billing activates tomorrow** | Key is set and `.env` is now actually read. Live calls work: a real image round-trips and returns a schema-valid observation, and given a synthetic image the model correctly declined rather than inventing a meal. Blocked only on credits clearing. **Free vision was rate-limited all day** — pin `google/gemini-3.7-flash` (~$0.003/photo). Accuracy remains unmeasured: every image tested was synthetic. |
...
| 5 | **Is the CGT curve shown to users at all?** | **Partly resolved 2026-09-08.** The dashboard card was showing the illustrative curve as a *measured sensor reading* under a hardcoded "Patch Connected" badge, with a fabricated 95 mg/dL default and "Postprandial trajectory optimal" — fixed in `53e58ce`; that card now reads `app.cgm_reading` or shows nothing. The CGT tab still shows the curve, correctly labelled illustrative. **The remaining question is yours**: keep the labelled curve, hide it until fitted, or drop it. |
