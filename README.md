...
| **Authentication** | Email + scrypt password, opaque bearer sessions revocable individually or everywhere. Credentials live in `app.user_credential`, separate from the pseudonymous `app.app_user` so analytics roles never see contact data. The old unverified header is dev-only and refused when `PATHYAM_ENV=production`. |
...
| **Rate limiting is per-account only** | A lockout counter after 8 failed attempts. There is no per-IP limit; that belongs at the edge, not in the app. | Phase 6 |
...
| **Consent is recorded, not enforced at write time** | Purposes are registered and granted (`POST /v1/auth/consent/{purpose}`), and `account`/`core_service` are recorded at sign-up. The telemetry endpoint does not yet refuse a write when `cgm_telemetry` is absent. | Phase 6 |
